...
define('DATABASE_PATH_STATS',$config['database.path.stats']);
...
define('LOL_PATCH_FULL',$api->staticData('versions')[0]);
...
class riotapi {
	...
	/* Internal Function */
	private function request($call, $dbPath, $dbFile, $dbTime = CACHE_LIFETIME_SECONDS, $region = null, $otherQueries = false, $static = false, $fullUrl = false) {
		if($fullUrl == true)
		{
			$url = $call;
		}
	...
	icon -> Full game summoner icons data
	...
	public function staticData($call, $fulldata = false, $locale = 'en_US', $version = null, $region='euw', $id=null) {
		...
		if(!empty($callIcon))
		{
			return $this->request('http://ddragon.leagueoflegends.com/cdn/'.LOL_PATCH_FULL.'/data/'.$locale.'/profileicon.json',$dbPath,$dbFile,$dbTime, $region, (strpos($call,'?') !== false), true, true);
		}
		else
		{
