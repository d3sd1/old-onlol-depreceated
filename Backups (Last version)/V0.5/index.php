        ...
        <div class="col-lg-6 col-sm-12 col-xs-12">
          <div class="news-slide m-b-15">
            <div class="vcarousel slide">
			  ...
			  <?php
			  $news = $db->query('SELECT id,title,image,cat,imageFullUrl,url FROM web_news WHERE lang="'.$userLang.'" ORDER BY updated DESC LIMIT 10');
			  $i = 0;
			  while($row = $news->fetch_row())
			  {
				  ...
				  {
						$imageUrl = URL.'/style/images/news/'.$row[2];
				  }
				  if($row[5] != null)
				  {
						$linkUrl = 'href="'.$row[5].'" target="ONLoLNews"';
				  }
				  else
				  {
						$linkUrl = 'href="'.URL.'/news/'.$row[0].'"';
				  }
				  if($i == 0)
				  {
					  $status = 'active ';
                  ...
                  <div class="overlaybg"><img src="'.$imageUrl.'"/></div>
                  <div class="news-content"><span class="label label-danger label-rounded">'.$row[3].'</span>
                    <h2><a '.$linkUrl.'>'.$row[1].'</a></h2>
                    <a '.$linkUrl.'>'.$lang['index.news.readmore'].'</a></div>
                </div>';
				$i++;
			  }
			  ?>
