		...
		foreach($summonerRecentGamesApi['games'] as $recentGameData)
		{
			...
			if(!empty($matchAction))
			{
				...
				if($matchAction == 'insert')
				{
					...
					$recentGameDataPlayerData['team'] = $recentGameData['teamId'];
					...
					$db->query('INSERT INTO api_matches VALUES ("'.$matchDataInfo['id'].'","'.$matchDataInfo['invalid'].'","'.$matchDataInfo['gameType'].'","'.$matchDataInfo['mapId'].'","'.$matchDataInfo['createDate'].'","'.$matchDataInfo['playerIds'].'","'.addslashes(json_encode($recentGameDataPlayerData)).'","'.$matchDataInfo['winner'].'")');
				}
				elseif($matchAction == 'update')
				{
					...
					$recentGamePreData['team'] = $recentGameData['teamId'];
