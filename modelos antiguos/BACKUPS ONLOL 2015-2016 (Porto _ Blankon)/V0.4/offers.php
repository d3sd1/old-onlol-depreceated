...
<div class="col-md-12">
                                            <div id="grid_54f5908ef15b8" class="grid-container grid-masonry container-fluid" data-item-style="tpl-masonry-style3" data-open-state="modal" data-column="2" data-size-gutter="10">
												...
												{
													echo '{"id":'.$skin_orderid.',"title":"'.$db->query('SELECT es_skin_name FROM lol_skins WHERE champname="'.champidtoname($row['champion_id']).'" AND skin_num="'.$row['skin_id'].'"')->fetch_row()[0].'","link":"'.URL.'/champions/'.champidtoname($row['champion_id']).'","post_type":"portfolio","thumb":"'.URL.'/style/images/base/champions/splash/'.champidtoname($row['champion_id']).'_'.$row['skin_id'].'.jpg","categories":[{"cat_id":1,"title":"'.$row['old_price'].' -> '.$row['new_price'].'","link":""}],"tags":[]}';
