...
class onlol{
	public function mmr($inv_id, $server, $lp, $league, $division)
	{
		global $db;
	...
	function updatesummoner_leagues($summonerid,$summoner,$region,$level,$icon,$lolrevision)
	{
		global $db;
		if((int) $level == 30)
		{
			/* Unranked */
			if(url_exists('https://'.$region.'.api.pvp.net/api/lol/'.$region.'/v2.5/league/by-summoner/'.$summonerid.'/entry?api_key='.LOL_API_KEY.'') == FALSE)
			{
			...
			else
			{
				/* Leagues */
				$actualteam = 0;
				$teams = readjson('https://'.$region.'.api.pvp.net/api/lol/'.$region.'/v2.5/league/by-summoner/'.$summonerid.'/entry?api_key='.LOL_API_KEY);
				$totalteams = count($teams[$summonerid]);
				
				while($actualteam < $totalteams)
				{
					$qeue = $teams[$summonerid][$actualteam]['queue'];
					$streakbase = $teams[$summonerid][$actualteam]['entries']['0']['isHotStreak'];
					$veteranbase = $teams[$summonerid][$actualteam]['entries']['0']['isVeteran'];
					$newbiebase = $teams[$summonerid][$actualteam]['entries']['0']['isFreshBlood'];
					$inactivebase = $teams[$summonerid][$actualteam]['entries']['0']['isInactive'];
					$tier = strtoupper(substr($teams[$summonerid][$actualteam]['tier'], 0,1));
					$division = parsedisivion($teams[$summonerid][$actualteam]['entries']['0']['division']);
					$lp = $teams[$summonerid][$actualteam]['entries']['0']['leaguePoints'];
					$name = $teams[$summonerid][$actualteam]['name']; /* Of actual team */
					$wins = $teams[$summonerid][$actualteam]['entries']['0']['wins'];
					$losses = $teams[$summonerid][$actualteam]['entries']['0']['losses'];
					/* Fix array data*/
					$streak = array();
					$veteran = array();
					$newbie = array();
	...
	public function updatesummoner_rankedgames($summonerid,$summoner,$region,$level,$league,$season = 'ACTUAL')
	{
	...
	public function updatesummoner_last10matches($summonerid,$region)
	{
		global $db;
	...
	public function updatesummoner_championskill($summonerid,$region,$level,$mmr)
	{
		global $db;
		if(date('m') == 01 or date('m') == 02)
		{
	...
	public function updatesummoner_mainchamp($summonerid,$region,$level)
	{
		global $db;
		if($level == 30 && $db->query('SELECT ranked_league FROM inv_users WHERE summoner_id="'.$summonerid.'" AND region="'.$region.'" LIMIT 1')->fetch_row()[0] != 'U')
		{
			if($db->query('SELECT id FROM inv_users_champskill WHERE summoner_id='.$summonerid.' AND region="'.$region.'"')->num_rows > 0)
			{
				$db->query('UPDATE inv_users SET main_champ="'.champidtoname(stdtoarray(json_decode($db->query('SELECT data FROM inv_users_champskill WHERE summoner_id='.$summonerid.' AND region="'.$region.'"')->fetch_row()[0]))[1]['champ_id']).'" WHERE region="'.$region.'" AND summoner_id="'.$summonerid.'"');
			}
			else
			{
                            ...
                            <li>
							 ...
							 <li>
                                ...
                                <ul>
                                    ...
                                    <li><a href="home-4.html">Páginas de maestrias</a></li>
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-sm-6 col-md-6 text-right">
                            ...
                            &nbsp;&nbsp;&nbsp;
                            <a href="" class="social_twitter no-style"></a>
                            <a href="" class="social_facebook no-style"></a>      
                        </div>
                    </div><!-- .row -->
                </div><!-- .container -->
            </div>';
...
function summonerupdate($name, $server, $redirect = false)
{
	...
	if(url_exists($summoner_info_url))
	{
		...
		$db->query('UPDATE inv_users SET onlol_last_update="'.time().'"');
		if($redirect != false)
		{
			redirect($redirect);
		}
	...
	{
		die('invocador no encontrado, url '.url_exists($summoner_info_url));
	}
}
...
function readjson($url)
{
	//se podrian cachear los archivos localmente para que fuese mas rapido
		...
		$result=curl_exec($ch);	$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		if($httpCode == 404) {
			return 'NOT_FOUND';
		}
