...
================================================== -->
		...
		<section class="content" style="background-image: url(<?php echo URL ?>/style/images/base/champions/splash/<?php echo $profile_mainchamp ?>_0.jpg); background-repeat: no-repeat; background-size: cover; background-attachment: fixed; min-height:100vh;">
         <div class="container" style="background:rgba(255,255,255,0.85); border-radius: 16px 16px 16px 16px; -moz-border-radius: 16px 16px 16px 16px; -webkit-border-radius: 16px 16px 16px 16px; border: 0px solid #000000;">
		  <div class="row">
			...
			<?php if(config('key_conflict_needactivation') == 'false') { ?>
                                    ...
                                    <div class="progress" style="background-color:#088A08;background-image: -webkit-linear-gradient(45deg, rgba(255, 255, 255, .15) 25%, transparent 25%, transparent 50%, rgba(102, 255, 51, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);background-image:      -o-linear-gradient(45deg, rgba(102, 255, 51, .15) 25%, transparent 25%, transparent 50%, rgba(102, 255, 51, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);background-image:         linear-gradient(45deg, rgba(102, 255, 51, .15) 25%, transparent 25%, transparent 50%, rgba(102, 255, 51, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);-webkit-background-size: 40px 40px;background-size: 40px 40px;-webkit-animation: progress-bar-stripes 2s linear infinite;-o-animation: progress-bar-stripes 2s linear infinite;animation: progress-bar-stripes 2s linear infinite;background-repeat: repeat-x;">
                                        <div class="progress-bar progress-bar-danger progress-bar-striped active" role="progressbar" aria-valuenow="<?php echo abs($ragefinal) ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo abs($ragefinal) ?>%;">
                                            <span><?php echo abs($ragefinal) ?>%</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="progress" style="background-color:#d9534f;background-image: -webkit-linear-gradient(45deg, rgba(201, 48, 44, .15) 25%, transparent 25%, transparent 50%, rgba(201, 48, 44, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);background-image:      -o-linear-gradient(45deg, rgba(201, 48, 44, .15) 25%, transparent 25%, transparent 50%, rgba(201, 48, 44, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);background-image:         linear-gradient(45deg, rgba(201, 48, 44, .15) 25%, transparent 25%, transparent 50%, rgba(201, 48, 44, .15) 50%, rgba(102, 255, 51, .15) 75%, transparent 75%, transparent);-webkit-background-size: 40px 40px;background-size: 40px 40px;-webkit-animation: progress-bar-stripes 2s linear infinite;-o-animation: progress-bar-stripes 2s linear infinite;animation: progress-bar-stripes 2s linear infinite;background-repeat: repeat-x;">
                                        <div class="progress-bar progress-bar-info progress-bar-striped active" role="progressbar" aria-valuenow="55" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo abs($motivationfinal) ?>%;">
                                            <span><?php echo abs($motivationfinal) ?>%</span>
                                        </div>
                                    </div>
			<?php } ?>
                                </div>
    ...
    <script>     
        ...
        function ajax_stream(){
		   ...
		   <div class="row">
			...
			if(@$_GET['menu'] == 'stats')
			{
												...
												if(is_array($profile_stats_data))
												{?>
		...
		$('#<?php echo $key ?>data').DataTable( {
			...
			{
				echo '<div class="col-md-4"><img width="50%" src="'.URL.'/style/images/not_found/thresh.png"></div><div class="col-md-8"><h1>¡Sin datos!</h1><h2>Pobres almas perdidas...<br>Parece ser que este invocador aún no ha jugado clasificatorias.</h2></div>';
			}
			}
			?>
			<?php
			...
			if(@$_GET['menu'] == 'badges')
			{
											...
											{
												echo '<table id="badgestable" class="display" cellspacing="-30px" width="100%">
				<thead >
					<tr>
        <th>#</th>
        <th>Título</th>
        <th>Descripción</th>
				...
				<tbody>';
												...
												while($row = $userbadges->fetch_row())
												{
													...
													if(strstr($row[0], 'mastery_player_of_'))
													{
														$depured = true;
														$fiximg = URL.'/style/images/base/champions/little/'.str_replace('mastery_player_of_', null, $row[0]).'.png';
													}
													if(empty($depured))
													{
													...
													echo '<tr>
															<td data-order="'.$row[0].'"><img draggable="false" width="75px" src="'.$fiximg.'"></td>
											...
											{
												echo '<div class="col-md-4"><img width="50px" src="'.URL.'/style/images/not_found/thresh.png"></div><div class="col-md-8"><h1>¡Sin datos!</h1><h2>Pobres almas perdidas...<br>Parece ser que este invocador aún no posee placas.</h2></div>';
											}
											?>
			<?php
			}
			...
			if(@$_GET['menu'] == 'runes')
			{
				...
				<div role="tabpanel" class="tabs-bordered" style="border:none;">
												 <?php
												  /* Menu */
												  echo ' <ul class="nav nav-tabs" style="background-color:rgba(255,255,255,0.2);">';
												  $actualrunepage_head = 1;
												  foreach($profile_runes_data as $getrunepagedata)
												  {
													if($getrunepagedata['current'] == 1)
													{
														$active_status = ' class="active"';
														$is_active_rune = ' - Activo';
													}
													else
													{
														$active_status = null;
													...
													echo '<li'.$active_status.'><a href="#'.$getrunepagedata['id'].'" data-toggle="tab">'.$actualrunepage_head.$is_active_rune.'</a></li>
													';
													$actualrunepage_head++;
												  }
														echo '</ul>';
												  ...
												  foreach($profile_runes_data as $getrunepagedatadetails)
												  {
													...
													while($actual_runepagecount < 30)
													{
													 ...
													 <div class="rune-page-runes" style="border: 1px solid #111; box-shadow: 0 0 5px #111 inset; background-image: url(\''.URL.'/style/images/base/summoners/runes/background.png\'); float: left; margin-left:8%; height: 436px; position: relative; width: 655px;">
                ...
                <div style="float: left; width: 250px; ">
                    <div style="background:rgb(17, 17, 17);border: 1px solid rgb(17, 17, 17); box-shadow: 0pt 0pt 5px rgb(17, 17, 17) inset; background: -moz-linear-gradient(center top , rgb(57, 57, 57) 0%, rgb(48, 48, 48) 100%) repeat scroll 0% 0% transparent; height: 436px;">
			...
			if(@$_GET['menu'] == 'champ_mastery')
			{
				...
				<tbody>
					<?php
					$profile_champ_mastery_data = $db->query('SELECT data FROM inv_users_champmastery WHERE summoner_id="'.$summonerid.'" AND region="'.$server_lookup.'"')->fetch_row();
					if(is_array($profile_champ_mastery_data))
					{
									foreach(stdtoarray(json_decode($profile_champ_mastery_data[0]))['champs'] as $data)
									{
										echo '<tr>
										...
										<td data-order="'.$data['mastery_points'].'">'.number_format($data['mastery_points'], 0, ',',',').'</td>
										<td data-order="'.$data['last_time_played'].'">Hace '.time_elapsed_string($data['last_time_played']).'</td>
										<td data-order="'.$data['mastery_points_need_nextlevel'].'">'.number_format($data['mastery_points_need_nextlevel'], 0, ',',',').'</td>
										<td data-order="'.$data['mastery_points_plus_lastlevel'].'">'.number_format($data['mastery_points_plus_lastlevel'], 0, ',',',').'</td>
