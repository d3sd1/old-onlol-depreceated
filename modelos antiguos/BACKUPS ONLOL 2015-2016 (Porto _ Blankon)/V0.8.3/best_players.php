...
<html lang="en">
    <head>
        <title>ONLoL ~ Mejores invocadores</title>
        <meta charset="utf-8">
			...
			<section class="content single padding2x" style="background-image: url(<?php echo URL ?>/style/images/home/bg/1.jpg); background-repeat: no-repeat; background-attachment: fixed;">
				...
				<tbody>
					<?php
									...
									$get_best_summs_all = $db->query('SELECT id,name,lp,region,wins,losses FROM lol_bestsummoners ORDER BY lp DESC LIMIT 200');
									...
									while($all_sums_count <= $all_sums_max && $all_inf = $get_best_summs_all->fetch_array(MYSQLI_ASSOC))
									{
										...
										echo '<tr>
										<td><img src="http://avatar.leagueoflegends.com/'.$all_inf['region'].'/'.$all_inf['name'].'.png" alt="Thumb"></td>
										<td>'.$all_inf['name'].'</td>
										<td>'.$all_inf['lp'].'</td>
