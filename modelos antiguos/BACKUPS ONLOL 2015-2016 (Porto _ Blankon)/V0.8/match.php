...
Duración: <form name="timing" style="margin-left:2%;"><input style="border:none;width:20px;" type="text" name="minutes" size="2" maxlength="2" value="'.$length_1.'">: <input style="border:none;width:20px;" type="text" name="seconds" size="2" maxlength="2" value="'.$length_2.'"></form>';
				...
				<div class="container-fluid">
					...
					$player_ids = null;
					$player_id_order = 0;
					foreach($game_info['participants'] as $value)
					{
						if($player_id_order != 0)
						{
							$player_ids .= ',';
						}
						$player_ids .= $value['summonerId'];
						$player_id_order++;
					}
					$summonersleaguedata = readjson('https://'.$server_lookup.'.api.pvp.net/api/lol/'.$server_lookup.'/v2.5/league/by-summoner/'.$player_ids.'/entry?api_key='.LOL_API_KEY);
					while($no_more_players == false)
					{
						if(!empty($game_info['participants'][$actualplayer]))
						{
							...
							{
								$just_actualized = true;
								if(!empty($summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']]))
								{
									$actualprofleague = strtoupper(substr($summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']][0]['tier'], 0,1));
									if($actualprofleague == 'U')
									{
										$actualprofleague = 1;
									}
									else
									{
										$actualprofleague = parsedisivion($summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']][0]['entries'][0]['division']);
									}
									$actualproflp = $summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']][0]['entries'][0]['leaguePoints'];
									$actualprofleaguename = $summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']][0]['name'];
									$actualprofdivision = parsedisivion($summonersleaguedata[$game_info['participants'][$actualplayer]['summonerId']][0]['entries'][0]['division']);
								}
								else
								{
									$actualprofleague = 'U';
									$actualprofdivision = 1;
									$actualproflp = 0;
									$actualprofleaguename = 'UNRANKED';
								}
								$db->query('INSERT INTO inv_users (summoner_id,name,region,ranked_league,ranked_division,icon,league_last_update,ranked_division_name,ranked_lp) VALUES ("'.$game_info['participants'][$actualplayer]['summonerId'].'","'.$game_info['participants'][$actualplayer]['summonerName'].'","'.$server_lookup.'","'.$actualprofleague.'","'.$actualprofdivision.'","'.$game_info['participants'][$actualplayer]['profileIconId'].'","'.time().'","'.$actualprofleaguename.'","'.$actualproflp.'")');
							}
							...
							{
								$actualprofdata = $db->query('SELECT ranked_league,ranked_division,ranked_lp FROM inv_users WHERE summoner_id='.$game_info['participants'][$actualplayer]['summonerId'].'')->fetch_row();
								if($actualprofdata[0] == 'U')
								{
									$is_unranked = null;
								}
								else
								{
									$is_unranked = parsedisivion($actualprofdata[1]);
								}
								$is_ranked = false;
								if($is_ranked == false)
								{
								...
								$construct_inforows = '<!-- Player '.$actualplayer.' '.$side.' -->
									...
									<div class="col-md-5" style="opacity:0.95; '.$team.'background:url('.URL.'/style/images/base/champions/splash/'.champidtokeyname($game_info['participants'][$actualplayer]['championId']).'_0.jpg);background-size: 100%;">
										<div class="row" style="padding-top:2%; padding-bottom:2%;">
											<a href="'.URL.'/champions/'.champidtokeyname($game_info['participants'][$actualplayer]['championId']).'"><div class="col-md-3">
												<img draggable="false" data-toggle="tooltip" data-placement="left" title="'.champidtoname($game_info['participants'][$actualplayer]['championId']).'" src="'.URL.'/style/images/base/champions/little/'.champidtokeyname($game_info['participants'][$actualplayer]['championId']).'.png">
											</div></a>
											<div class="col-md-1" style="float:left; margin-left:-5%;">
												<div class="row"><img data-toggle="tooltip" data-placement="right" title="'.spellidtoname($game_info['participants'][$actualplayer]['spell1Id']).'" draggable="false" width="60px" src="'.URL.'/style/images/base/summoners/spells/'.$game_info['participants'][$actualplayer]['spell1Id'].'.png"></div>
												<div class="row"><img data-toggle="tooltip" data-placement="right" title="'.spellidtoname($game_info['participants'][$actualplayer]['spell2Id']).'" draggable="false" width="60px" src="'.URL.'/style/images/base/summoners/spells/'.$game_info['participants'][$actualplayer]['spell2Id'].'.png"></div>
											</div>
											<div class="row">
												<div class="col-md-2" style="margin-left:0%;margin-top:'.$nameboxmargin.';">
													<img draggable="false" width="60px" src="'.URL.'/style/images/base/summoners/icon/'.$game_info['participants'][$actualplayer]['profileIconId'].'.png">
												</div>
												<a href="'.URL.'/summoner/'.$server_lookup.'/'.$game_info['participants'][$actualplayer]['summonerName'].'"><div class="col-md-4" style="margin-top:'.$nameboxmargin.'; margin-left:-6.5%; height: 60px; background:white;border-radius: 0px 21px 21px 0px;-moz-border-radius: 0px 21px 21px 0px;-webkit-border-radius: 0px 21px 21px 0px;border: 0px solid #000000;">
													'.parsename_lookup($game_info['participants'][$actualplayer]['summonerName']).'
												</div></a>
												<div class="col-md-2" style="text-align:center;margin-left:2%; background:white;border-radius: 19px 19px 19px 19px;-moz-border-radius: 19px 19px 19px 19px;-webkit-border-radius: 19px 19px 19px 19px;border: 0px solid #000000;opacity:0.80;">
													<img data-toggle="tooltip" data-placement="right" title="'.$game_info['participants'][$actualplayer]['summonerName'].' está clasificado como '.leaguetotxt($actualprofdata[0]).' '.parsedisivion($actualprofdata[1]).' y cuenta con '.$actualprofdata[2].' puntos de liga." draggable="false" style="opacity:1.4;" width="100px" src="'.URL.'/style/images/base/summoners/division/'.$actualprofdata[0].'/'.$actualprofdata[1].'.png">
													<h4 style="margin-top:-5%;">'.$is_unranked.'</h4>
												</div>
