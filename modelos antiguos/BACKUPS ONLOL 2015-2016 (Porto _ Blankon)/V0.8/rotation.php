...
<div class="col-md-12">
                                            <div id="grid_54f5908ef15b8" class="grid-container grid-masonry container-fluid" data-item-style="tpl-masonry-style3" data-open-state="modal" data-column="2" data-size-gutter="10">
												...
												<?php
												$rotationchamps = $db->query('SELECT id,champname,champ_keyname,title,role_1,role_2 FROM lol_champs WHERE is_rotation="1" LIMIT 10') or die($db->error);
												$champ_orderid = 1;
												while($row = $rotationchamps->fetch_array(MYSQL_ASSOC))
												{
													$role_1 = parserole($row['role_1']);
													if($row['role_2'] != 'null')
													{
														$role_2 = parserole($row['role_2']);
													}
													else
													{
														$role_2 = null;
													}
													$roleinput = '{"cat_id":1,"title":"'.$role_1.'","link":"'.URL.'/champions"}';
													if($role_2 != null)
													{
														$roleinput .= ',{"cat_id":2,"title":"'.$role_2.'","link":"'.URL.'/champions"}';
													}
													echo '{"id":'.$champ_orderid.',"title":"'.$row['champname'].', '.$row['title'].'","link":"'.URL.'/champions/'.$row['champ_keyname'].'","post_type":"portfolio","thumb":"'.URL.'/style/images/base/champions/splash/'.$row['champ_keyname'].'_0.jpg","categories":['.$roleinput.'],"tags":[]}';
													if($champ_orderid != 10)
													{
														echo ',';
														$champ_orderid++;
