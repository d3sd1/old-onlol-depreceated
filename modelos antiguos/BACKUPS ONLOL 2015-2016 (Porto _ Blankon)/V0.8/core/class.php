...
class onlol{
	...
	public static function checkingame($region,$summonerid)
	{
		...
		{
			$game_info = readjson('https://'.$region.'.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/'.$lol_servers[$region].'/'.$summonerid.'?api_key='.LOL_API_KEY);
			...
			else
			{
				foreach($game_info['participants'] as $data)
				{
					if($data['bot'] == 'false')
					{
						...
						{
							$db->query('INSERT INTO inv_users (summoner_id,icon,name) VALUES ("'.$data['summonerId'].'","'.$data['profileIconId'].'","'.$data['summonerName'].'")');
						}
						else
						{
							$db->query('UPDATE inv_users SET ingame_last_check='.time().',is_ingame="1" WHERE summoner_id="'.$data['summonerId'].'"');
						}
					}
				}
				return 'INGAME';
