			...
			if(empty($reload_common_data) or @$reload_common_data != false)
			{
					...
					if((int) $level == 30)
					{
							...
							while($this_summonerteam_updating < count($summonerteams_ids) && is_array($teams))
							{
								if(array_key_exists($summonerteams_ids[$this_summonerteam_updating],$teams))
								{
									...
									{
										$entry = array_search($summoner_name, array_column($teams[$summonerteams_ids[$this_summonerteam_updating]][$actualteam]['entries'], 'playerOrTeamName'));	
										...
										{
											if($summoner_id == $summonerteams_ids[$this_summonerteam_updating])
											{
												$pre_leaguedata = $db->query('SELECT ranked_wins,ranked_losses FROM inv_users WHERE summoner_id='.$summoner_id.' AND region="'.$region.'"')->fetch_row();
												if($pre_leaguedata[0] != $wins OR $pre_leaguedata[1] != $losses)
												{
													$load_rkddata = 1;
												}
											}
				...
				if($task == 5)
				{
					...
					if(is_array($champ_mastery))
					{
						...
						$db->query('UPDATE inv_users SET champ_mastery="'.addslashes(json_encode($champ_mastery_db)).'" WHERE summoner_id='.$summoner_id);
					}
