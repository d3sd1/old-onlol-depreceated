...
class riot{
	...
	public static $url_actualgame_by_summonerid = 'https://{{region}}.api.pvp.net/api/lol/{{region}}/v1.4/summoner/{{summoner_id}}/runes?api_key='.LOL_API_KEY;
	public static $url_runes_by_summonerid = 'https://{{region}}.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/{{region_code}}/{{summoner_id}}?api_key='.LOL_API_KEY;
}
	class check extends riot{
		public static function ingame($summoner_id,$region)
		{
			global $lol_servers;
			if(onlol::readjson(str_replace(array('{{region}}','{{region_code}}','{{summoner_id}}'),array($region,strtoupper($lol_servers[$region]),$summoner_id),self::$url_runes_by_summonerid)) == 'NOT_FOUND')
			{
				return false;
			}
			else
	...
	class reload extends riot{
		...
		public static function recent_matches($summoner_id,$region)
		{
			...
			$recent_games_playedwith = $db->query('SELECT summoner_ids FROM lol_matches WHERE summoner_ids LIKE "%'.$summoner_id.'%" AND creation_timestamp > '.(time()-onlol::config('played_with_interval')));
			...
			if(!empty($add_summoner_to_db))
			{
				...
				foreach($summoners_to_add_chunk as $count => $add_summoner_to_db)
				{
					...
					$summoner_info = onlol::readjson(str_replace(array('{{region}}','{{summoners_query}}'),array($region,onlol::api_format_name($summoners_to_add)),self::$url_league_by_summoner));
					if(is_array($summoner_info))
					{
