...
class template{
	...
	{
		if(onlol::config('inactivity_enabled') == 'true')
		{
			if(empty($_SESSION['onlol_inactivity_disabled']))
			{
				return '<meta http-equiv="Refresh" content="'.onlol::timing('inactivity_interval').';url='.URL.'/start?inactivity=true">';
			}
		}
	}
	public static function nav_top()
	{
		$return = template::inactivity().' <!-- START @HEADER -->
            <header id="header">
                        ...
                        <li id="tour-2" class="dropdown navbar-notification">
                            ...
                            <div class="dropdown-menu animated bounce">
                                ...
                                <div class="dropdown-body niceScroll">
                                    ...
                                    <!-- Start notification list -->
                                    <div class="media-list small">';
									  foreach($GLOBALS['lol_servers'] as $region => $region_code)
									  {
										$point_color = 'black';
										if(lol::shards($region) == 0)
										{
											$point_color = 'red';
										}
										if(lol::shards($region) == 1)
										{
											$point_color = 'yellow';
										}
										if(lol::shards($region) == 2)
										{
											$point_color = 'green';
										}
										$return .= '<a href="'.URL.'/region/'.$region.'" class="media">
                                            <div class="media-body">
                                                <span class="media-text" style="vertical-align:middle;"><i class="fa fa-circle" style="color:'.$point_color.'; font-size: 12px;" aria-hidden="true" data-toggle="tooltip" title="'.lang::trans('region_shard_status').': '.lang::trans('region_shard_status_'.lol::shards($region)).'" data-placement="right"></i> <b>'.strtoupper($region).'</b> ('.lang::trans('region_name_'.$region).')</span>
                                            </div><!-- /.media-body -->
                                        </a><!-- /.media -->';  
									  } 
	...
	{
		$return = '<aside id="sidebar-left" class="sidebar-rounded">

                <!-- Start left navigation - profile shortcut -->
                <div class="sidebar-content">
                    <div id="tour-5" class="media">';
					if(!empty($_SESSION['default_summoner']) && @$_SESSION['default_summoner']['name'] != null)
					{
						$summoner_ingame_status = null;
						if(check::ingame(lol::summoner_name2id($_SESSION['default_summoner']['name'],$_SESSION['default_summoner']['region']),$_SESSION['default_summoner']['region']) == true)
						{
							$summoner_ingame_status = '<i class="online"></i>';
						}
						$return .= '<a class="pull-left has-notif avatar" href="'.URL.'/summoner/'.$_SESSION['default_summoner']['region'].'/'.$_SESSION['default_summoner']['name'].'" data-toggle="tooltip" data-placement="right" data-title="'.lang::trans('default_user_tooltip').'" data-original-title="" >
                            <img src="'.URL.'/game/icons/'.$_SESSION['default_summoner']['icon'].'.png" alt="default">
							'.$summoner_ingame_status.'
						</a>
                        <div class="media-body">
                            <h4 class="media-heading">'.$_SESSION['default_summoner']['name'].'</h4>
                            <small>'.strtoupper($_SESSION['default_summoner']['region']).'</small>
                        </div>';
					}
					else
					{
                ...
                <ul class="sidebar-menu" id="tour-10">';
                    ...
                    <li class="submenu active">
                        ...
                        <ul>
                            <li><a href="'.URL.'/home">'.lang::trans('left_menu_basepage').'</a></li>
                            <li><a href="dashboard-ecommerce.html">'.lang::trans('left_menu_advsearch').'</a></li>
                            <li><a href="'.URL.'/contact">'.lang::trans('left_menu_contact').'</a></li>
                        </ul>
                    </li>
                ...
                <div class="sidebar-footer hidden-xs hidden-sm hidden-md">';
                    ...
                    <a id="tour-14" class="pull-left" href="'.URL.'/start?force=true" data-toggle="tooltip" data-placement="top" data-title="'.lang::trans('submenu_go_index').'"><i class="fa fa-home"></i></a>
                </div><!-- /.sidebar-footer -->
