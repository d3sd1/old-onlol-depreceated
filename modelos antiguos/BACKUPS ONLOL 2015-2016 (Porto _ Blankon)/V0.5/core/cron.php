	...
	public function wipe_info_champions()
	{
		if($this->timer('champinfo') == TRUE)
		{
			...
			while($done_champs < count($champ_data['data']))
			{
				...
				$champ_info = readjson('http://ddragon.leagueoflegends.com/cdn/'.$this->lol_version.'/data/es_ES/champion/'.$champ_name.'.json');
				// PASSIVE
						$db->query('INSERT INTO lol_champs_skills (passive_name,passive_description,champ_keyname'.$p_colum.') VALUES ("'.$champ_info['data'][$champ_name]['passive']['name'].'","'.addslashes($champ_info['data'][$champ_name]['passive']['description']).'","'.$champ_key.'"'.$p_cooldowns.')') or die($db->error);
					
				// Q
					if(isset($champ_info['data'][$champ_name]['spells']['0']['cooldown']))
					{
						$q_colum = ',q_cooldown="'.implode('/',$champ_info['data'][$champ_name]['spells']['0']['cooldown']).'"';
					}
					$db->query('UPDATE lol_champs_skills SET q_name="'.$champ_info['data'][$champ_name]['spells']['0']['name'].'", q_description="'.addslashes($champ_info['data'][$champ_name]['spells']['0']['description']).'" '.$q_colum.' WHERE champ_keyname="'.$champ_key.'"') or die($db->error);
			
				// W
					if(isset($champ_info['data'][$champ_name]['spells']['1']['cooldown']))
					{
						$w_colum = ',w_cooldown="'.implode('/',$champ_info['data'][$champ_name]['spells']['1']['cooldown']).'"';
					}
					$db->query('UPDATE lol_champs_skills SET w_name="'.$champ_info['data'][$champ_name]['spells']['1']['name'].'", w_description="'.addslashes($champ_info['data'][$champ_name]['spells']['1']['description']).'" '.$w_colum.' WHERE champ_keyname="'.$champ_key.'"') or die($db->error);
			
				// E
					
					if(isset($champ_info['data'][$champ_name]['spells']['2']['cooldown']))
					{
						$e_colum = ',e_cooldown="'.implode('/',$champ_info['data'][$champ_name]['spells']['2']['cooldown']).'"';
					}
					$db->query('UPDATE lol_champs_skills SET e_name="'.$champ_info['data'][$champ_name]['spells']['2']['name'].'", e_description="'.addslashes($champ_info['data'][$champ_name]['spells']['2']['description']).'", champ_keyname="'.$champ_key.'" '.$e_colum.' WHERE champ_keyname="'.$champ_key.'"') or die($db->error);
			
				// R
					if(isset($champ_info['data'][$champ_name]['spells']['3']['cooldown']))
					{
						$r_colum = ',r_cooldown="'.implode('/',$champ_info['data'][$champ_name]['spells']['3']['cooldown']).'"';
					}
					$db->query('UPDATE lol_champs_skills SET r_name="'.$champ_info['data'][$champ_name]['spells']['3']['name'].'", r_description="'.addslashes($champ_info['data'][$champ_name]['spells']['3']['description']).'", champ_keyname="'.$champ_key.'" '.$r_colum.' WHERE champ_keyname="'.$champ_key.'"') or die($db->error);
