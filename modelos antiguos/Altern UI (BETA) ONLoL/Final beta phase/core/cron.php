...
class cron
{
	...
	public function exec_onlol_cron()
	{
		...
		$this->wipe_info_champions();
		$this->wipe_stats_mmr();
		$this->bestplayers_champions();*/$this->wipe_info_game();
	}
	...
	public function update_actual_summoners()
	{
		if($this->timer('reloadinvs') == TRUE)
		{
			global $db;
			updating('SUMMONERS');
			$summoners = $db->query('SELECT summoner_id,region FROM inv_users WHERE onlol_last_update<'.(time()-config('cron_inv_reload')).'');
			/* Update only matches because it would be lost data */
			while($row = $summoners->fetch_row())
			{
				onlol::updatesummoner_last10matches($row[0],$row[1]);
			}
